...
void fitlin(){
  ...
  minuit->SetFCN(fcn);             //SetFCN è il metodo e prende come valore fcn
  minuit->DefineParameter(0,"a",4,0.01,0.,10.);
  minuit->DefineParameter(1,"b",0,0.01,0.,0.);

  // Minimize
  minuit->Command("MIGRAD");  //chiama la funzione fornita fcn e procede alla minimizzazione
